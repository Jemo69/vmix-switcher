...
class SwitcherApp {
  ...
  // Thumbnail feed badge: names the SOURCE TYPE (NDI / CAM / VIDEO / SCREEN),
  // never the tally. Tally lives only in the status bar (PROGRAM / PREVIEW /
  // STANDBY) so a standby input can never wear a LIVE badge.
  feedBadgeLabel(inp) {
    ...
    if (t.includes('mix') || t.includes('output')) return 'MIX';
    return 'FEED';
  }
  ...
  renderMultiviewGrid(inputs) {
    ...
    if (canUpdateInPlace) {
      existingTiles.forEach((tile, idx) => {
        ...
        if (badgeEl) {
          const liveDot = inp.isLiveSource ? `<span class="live-signal-dot ${inp.signalStatus === 'live' ? 'live' : ''}"></span>` : '';
